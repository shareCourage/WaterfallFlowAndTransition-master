...
- (void)loadData {
    ...
    self.index++;
    // 设置布局的属性
    self.waterfallFlowLayout.columnCount = 4;
...
{
    LNGood *good = self.goodsList[indexPath.item];
    NSLog(@"good -> %@",good.price);
    UIStoryboard *sb = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    PHTransitionController *transition = [sb instantiateViewControllerWithIdentifier:@"transition"];
    transition.good = good;
//    transition = [[PHTransitionController alloc] init];
    if (transition) {
...
{
    if ([toVC isKindOfClass:[PHTransitionController class]]) {
        NSLog(@"viewController -> PHTransitionController");
        MagicMoveTransition *magic = [[MagicMoveTransition alloc] init];
        return magic;
    }
    return nil;
}
